...
function gpc_val($val, $runfunc, $emptyrun)
{
    ...
    if ($runfunc && strpos($runfunc, '|')) {
        ...
        foreach ($funcs as $run) {
            if ('xss' == $run) {
                $val = \Sockphp\Helper\Xss::getInstance()->clean($val);
            } else {
                $val = $run($val);
            }
        }
    ...
    if ('xss' == $runfunc) {
        return \Sockphp\Helper\Xss::getInstance()->clean($val);
    }
    if ($runfunc) {
        return $runfunc($val);
    }
...
function getini($key)
{
    $_CFG = \Sockphp\App::mergeVars('cfg');
    $k = explode('/', $key);
    switch (count($k)) {
        case 1:
